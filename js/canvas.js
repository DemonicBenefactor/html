...
if (window.addEventListener) {
  window.addEventListener('load', function() {
    ...
    function init() {
      ...
      if (!canvas) {
        alert("Error, cannot create canvas element.")
        return;
      }
      ...
      // Fill transparent canvas with dark grey (So we can use the color to
      // erase).
    ...
    // Create the temporary canvas on top of the canvas, which is cleared each
    // time the user draws.
    function img_update() {
    ...
    tools.rect = function() {
      ...
      this.mousemove = function(ev) {
        ...
        var x = Math.min(ev._x, tool.x0), y = Math.min(ev._y, tool.y0),
            w = Math.abs(ev._x - tool.x0), h = Math.abs(ev._y - tool.y0);
        context.clearRect(0, 0, canvas.width,
                          canvas.height); // Clears the rectangle onload.
